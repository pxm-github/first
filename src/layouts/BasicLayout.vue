<template>
  ...
  >
    ...
    <template v-slot:menuHeaderRender>
      <div class="menuHeader" @click="goHome">
        <!-- <logo-svg /> -->
        <!-- <h1>{{ title }}</h1> -->
        {{ title }}
      </div>
    ...
    <!-- <template v-slot:headerContentRender>
      <div>
        <a-tooltip title="刷新页面">
          <a-icon type="reload" style="font-size: 18px;cursor: pointer;" @click="locationView" />
        </a-tooltip>
      </div>
    </template> -->
...
import { roleDefulRouter } from "@/utils/defulRouter";
import {
  CONTENT_WIDTH_TYPE,
  SIDEBAR_TYPE,
  TOGGLE_MOBILE_TYPE,
...
export default {
  ...
  methods: {
    ...
    goHome() {
      // let redirect = roleDefulRouter(this.roles.permissionList[0]);
